import type { StoreDescriptor, ExpirableSchema, CacheKey, ExpirableValue } from '@/types/storage'
...
import { defaultDuration } from '@/requests/modules/cache/constants'
...
export class ExpirableCacheStorage<R = any>{
     ...
     */
    getCache(key: CacheKey): R | undefined {
        const expirableValue = this.store.getItem(key) as ExpirableValue<R> | undefined
        if (!expirableValue) {
            return void 0
        }
     ...
     */
    hasValidCache(key: CacheKey): boolean {
        const expirableValue = this.store.getItem(key) as ExpirableValue<R> | undefined
        if (!expirableValue) {
            return false
        }
