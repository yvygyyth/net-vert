...
import { createMockRequestor, createFailNTimesMockRequestor, delay, type Data } from './test-utils';
    ...
    describe('并发 + 重试组合', () => {
        it('应该限制并发数量并在失败时重试', async () => {
            ...
            const promises = Array.from({ length: 5 }, (_, i) =>
                requestor.get<Data>(`/api/test-${i}`).catch(() => ({ error: true })),
            );
        ...
        it('应该对并发池中的每个请求独立重试', async () => {
            ...
            const requestor = createRequestor({
                extensions: [concurrent({ parallelCount: 2 }), retry({ retries: 2, delay: 0 })],
            });
