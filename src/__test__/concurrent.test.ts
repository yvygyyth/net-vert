...
describe('并发模块测试', () => {
    ...
    it('应该限制并发数量', async () => {
        ...
        const requestor = createRequestor({
            extensions: [concurrentMiddleware],
        });

        // 同时发起 6 个请求
        const promises = Array.from({ length: 6 }, (_, i) => requestor.get<Data>(`/api/test-${i}`));
    ...
    it('应该按顺序执行所有任务', async () => {
        ...
        const requestor = createRequestor({
            extensions: [concurrent({ parallelCount: 3 })],
        });

        const results: number[] = [];
        const promises = Array.from({ length: 5 }, (_, i) =>
            requestor.get<Data>(`/api/test-${i}`).then(() => results.push(i)),
        );
    ...
    it('应该正确处理任务失败的情况', async () => {
        ...
        const requestor = createRequestor({
            extensions: [concurrent({ parallelCount: 2 })],
        });
    ...
    it('应该使用自定义 ID 创建器', async () => {
        ...
        const promises = Array.from({ length: 3 }, (_, i) => requestor.get<Data>(`/api/test-${i}`));
    ...
    it('应该在高并发下稳定工作', async () => {
        ...
        const requestor = createRequestor({
            extensions: [concurrent({ parallelCount: 5 })],
        });
