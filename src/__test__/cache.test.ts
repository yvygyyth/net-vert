...
import { createMockRequestor, TEST_UTILS_MOCK_KEY } from './test-utils';
...
describe('缓存模块测试', () => {
    ...
    it('应该测试缓存功能：首次请求正常执行并缓存', async () => {
        ...
        const result1 = await requestor.get('/api/users');
        const result2 = await requestor.get('/api/users');
    ...
    it('应该测试缓存功能：缓存过期后缓存失效', async () => {
        ...
        const result2 = await requestor.get<{ timestamp: number }>('/api/users');
    ...
    it('应该测试缓存功能：自定义 key 生成函数，不同参数使用不同缓存', async () => {
        ...
        // 相同 URL，不同参数
        const result1 = await requestor.get('/api/users', { params: { id: 1 } });
        const result2 = await requestor.get('/api/users', { params: { id: 2 } });
    ...
    it('应该测试缓存功能：自定义 isValid 校验，根据业务逻辑判断缓存有效性', async () => {
        ...
        const result3 = await requestor.get('/api/users');
    ...
    it('应该测试缓存功能：POST 请求带不同 data，使用不同缓存', async () => {
        ...
        // 相同 URL，不同 data
        const result1 = await requestor.post('/api/users', { name: 'Alice' });
        const result2 = await requestor.post('/api/users', { name: 'Bob' });
        const result3 = await requestor.post('/api/users', { name: 'Alice' }); // 与第一次相同
