...
interface RetryOptions<D = any> {
     ...
     * 默认：0
     */
    delay?: number | ((ctx: { config: RequestConfig<D>; lastResponse: any; attempt: number }) => number)
     ...
     * 默认：所有错误都重试
     */
    retryCondition?: (ctx: { config: RequestConfig<D>; lastResponse: any; attempt: number }) => boolean
}
```
